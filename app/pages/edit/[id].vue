...
if (error.value) {
    toast.add({ title: `Error: ${error.value.code}`, description: error.value.message, color: 'error' })
} else if (data.value) {
...
const onSubmit = async (event: FormSubmitEvent<IUpdateGroup>) => {
    loading.value = true
    const { error } = await supabase.from('groups').update(event.data).eq('id', id)

    if (error) {
        toast.add({ title: `Error: ${error.code}`, description: error.message, color: 'error' })
    } else {
        toast.add({ title: 'Success', description: 'Group updated successfully', color: 'success' })
        router.push('/')
    }
...
<template>
    <section class="py-12">
        <UCard>
            <UForm id="update-group-form" :schema="updateGroupSchema" :state="editGroupData" class="space-y-4"
                @submit="onSubmit">
